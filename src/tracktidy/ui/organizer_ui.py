"""
Audio file organizer UI for TrackTidy
"""
import os
import asyncio
...
from src.tracktidy.core.organizer import organize_audio_files_by_format

console = Console()

async def organizer_ui():
    """
    User interface for organizing audio files by format
    """
    while True:
        console.clear()
        
        # Display header
        header = Text("🗂️ Audio File Organizer 🗂️", style="bold #f5c2e7")
        console.print(Panel(header, style="bold #cba6f7"))
        
        console.print("\n[bold #89b4fa]This tool helps you organize your audio files by format.[/bold #89b4fa]")
        console.print("[#89dceb]Files will be moved to separate folders based on their format (mp3_files, wav_files, etc.)[/#89dceb]\n")
        
        # Display organizer options
        console.print("[#89b4fa]1.[/#89b4fa][bold] Organize Audio Files by Format[/bold]")
...
async def organize_files():
    ...
    console.print(f"\n[bold #fab387]This will organize all audio files in {directory} into format-specific folders.[/bold #fab387]")
    ...
    # Organize the files
    console.print("\n[bold #89b4fa]Organizing audio files...[/bold #89b4fa]")
    
    try:
        success, stats, errors = await organize_audio_files_by_format(directory)
        
        if success:
            console.print(f"\n[bold #a6e3a1]✅ Successfully organized {stats['moved_files']} of {stats['total_files']} audio files![/bold #a6e3a1]")
